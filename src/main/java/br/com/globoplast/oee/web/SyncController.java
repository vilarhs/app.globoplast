...
public class SyncController {
    ...
    @GetMapping("/health") public Map<String,Object>health(){return Map.of("ok",true,"servico","globoplast","versao",AppConfig.VERSION);}
    @GetMapping("/java-sync/v1/health") public Map<String,Object>javaSyncHealth(){return Map.of("ok",true,"servico","globoplast","versao",AppConfig.VERSION);}
    @PostMapping(path={"/sync/v1/status","/java-sync/v1/status"},consumes=MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)public ResponseEntity<?>status(@RequestBody byte[]body,HttpServletRequest req){if(!auth(body,req))return error(HttpStatus.UNAUTHORIZED,"Autenticação inválida");return ResponseEntity.ok(Map.of("ok",true,"servico","globoplast","versao",AppConfig.VERSION,"banco",AppConfig.dbFile().toString(),"status",sync.status()));}
