...
public class SyncService {
    ...
    public Map<String,Object> importBatch(String source,List<Map<String,Object>>records,String connectorId,String sentAt){
        ...
        for(Map<String,Object>r:records){
            long id=longVal(r.get("erp_id"));
            if(id==0||(src.equals("refugo")&&id<0))throw new IllegalArgumentException(src.toUpperCase()+" sem erp_id válido");
            unique.put(id,r);
        }
        String now=ZonedDateTime.now(AppConfig.ZONE).toString();int changed=0;
        try(Connection c=db.open()){
            c.setAutoCommit(false);
            for(Map.Entry<Long,Map<String,Object>>e:unique.entrySet()){
                long id=e.getKey();Map<String,Object>r=e.getValue();String hash=hash(r);String old=null;
                try(PreparedStatement q=c.prepareStatement("SELECT payload_hash FROM "+table+" WHERE erp_id=?")){q.setLong(1,id);ResultSet rs=q.executeQuery();if(rs.next())old=rs.getString(1);}
            ...
            long total;Long max;
            try(Statement s=c.createStatement();ResultSet rs=s.executeQuery("SELECT COUNT(*),MAX(erp_id) FROM "+table)){rs.next();total=rs.getLong(1);max=rs.getObject(2)==null?null:rs.getLong(2);}
    ...
    private static String tableForSource(String source){
    ...
    private static String date(Object v,String field){var d=Norm.isoDate(v);if(d==null)throw new IllegalArgumentException("Registro sem "+field);return d.toString();}
